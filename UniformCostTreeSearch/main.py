...
class Node:
    ...
    # Compare nodes based on path cost and then by vacuum position
    def __lt__(self, other):
        if self.path_cost == other.path_cost:
            # First consider row (x-coordinate)
            if self.vacuum_pos[0] == other.vacuum_pos[0]:
                # Then consider column (y-coordinate)
                return self.vacuum_pos[1] < other.vacuum_pos[1]
            return self.vacuum_pos[0] < other.vacuum_pos[0]
...
def uniform_cost_tree_search(start_pos, dirty_squares):
    ...
    fringe = []
    heapq.heappush(fringe, start_node)  # Add start node directly to the fringe
