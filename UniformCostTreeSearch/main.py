...
# # Uniform Cost Tree Search implementation with a timer
# def uniform_cost_tree_search(start_pos, dirty_squares):
#     # Priority queue (fringe) initialized with the start node
...
#     last_report_time = start_time  # To track the last time a message was printed
#     report_interval = 10  # Seconds between time reports
...
#         # Check time elapsed and report if necessary
#         current_time = time.time()
#         if current_time - last_report_time >= report_interval:
#             elapsed_time = int(current_time - start_time)
#             print(f"Time elapsed: {elapsed_time} seconds... still searching.")
...
#     while node:
#         if node.action:
#             path.append((node.action, node.vacuum_pos))
#         node = node.parent
#     return path[::-1]  # Return reversed path (from start to goal)

# # Initial problem configuration
# start_pos = (2, 2)  # Vacuum starts at (2, 2)
# #dirty_squares = {(1, 2), (2, 4), (3, 5)}  # Initial dirty squares
# dirty_squares = {(1, 2), (2, 4), (3, 5)}  # Initial dirty squares

# # Perform the search
# solution = uniform_cost_tree_search(start_pos, dirty_squares)
...
#         if current_time - last_report_time >= report_interval:
#             elapsed_time = int(current_time - start_time)
#             print(f"\nTime elapsed: {elapsed_time} seconds... still searching.")
            
#             # Print the current fringe state
#             print("Fringe content (cost, vacuum position, dirty squares):")
#             for cost, node in fringe:
#                 print(f"Cost: {cost}, Position: {node.vacuum_pos}, Dirty squares: {node.dirty_squares}")
...
#         new_x, new_y = new_pos
#         # Check bounds (stay within the 4x5 grid)
#         if action != 'SUCK' and (0 <= new_x < 4 and 0 <= new_y < 5):
#             # Move to a valid position within the grid
#             successors.append((action, (new_pos, dirty_squares), cost))
#         elif action == 'SUCK' and node.vacuum_pos in dirty_squares:
#             # Remove the dirty square if 'SUCK' is performed
...
# # Initial problem configuration
# start_pos = (2, 2)  # Vacuum starts at (2, 2)
# dirty_squares = {(1, 2)}  # Initial dirty squares
...
#             return reconstruct_path(current_node), total_time
...
#     for action, (new_pos, cost) in actions.items():
#         new_x, new_y = new_pos
#         # Check bounds (stay within the 4x5 grid, corners: (1,1), (4,1), (1,5), (4,5))
#         if action != 'SUCK' and (1 <= new_x <= 4 and 1 <= new_y <= 5):
#             # Move to a valid position within the grid
#             successors.append((action, (new_pos, dirty_squares), cost))
#         elif action == 'SUCK' and node.vacuum_pos in dirty_squares:
...
# # Perform the search
# solution, total_time = uniform_cost_tree_search(start_pos, dirty_squares)

# if solution:
#     print("Solution path:")
#     for step in solution:
#         print(f"Action: {step[0]}, Vacuum position: {step[1]}")
#     print(f"Total time to find solution: {total_time:.2f} seconds")
# else:
#     print("No solution found.")
